...
DELIMITER $$
CREATE DEFINER=`root`@`10.102.50.174` PROCEDURE `archive_lapp_meta`(IN `lapp_stage` VARCHAR(100), IN `archive_date` DATE)
    NO SQL
BEGIN
    SET NAMES utf8mb4 COLLATE utf8mb4_unicode_ci;
	SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
...
DELIMITER $$
CREATE DEFINER=`root`@`10.102.50.174` PROCEDURE `sp_seed_loan_dataset`()
BEGIN
set @id=0;

UPDATE data_store_config SET process_restart_id = 0 ,job_last_id = 0 ,job_token = '' WHERE data_store_id = 'loan_dataset_seed' ;

truncate table data_changes.loan_data_seed;
...
select distinct sublan_id from loan_entries where sublan_id !='lan' limit 10 ;

truncate table data_changes.sublan_data_seed;

insert ignore into data_changes.sublan_data_seed(stamp,object_id,activity,object_type)
select RIGHT(CONCAT('00000000000', row_number() over()),12) as stamp,object_id,'changed','sublan' from t_objects;

select  @id:=max(id) from data_changes.sublan_data_seed;

WHILE (@id > 0) DO
	select @sublan_id:=object_id from data_changes.sublan_data_seed limit 1;

	call sp_loan_dataset(@sublan_id);
	
	delete from data_changes.sublan_data_seed where object_id=@sublan_id;
select @id;
...
SET NAMES utf8mb4 COLLATE utf8mb4_unicode_ci;
	...
	CREATE TEMPORARY TABLE t_loan_daily_obs (
		...
		added_instalment decimal(11,0) DEFAULT 0,
		added_penalty_dues decimal(11,0) DEFAULT 0,
		added_penalty_waiver decimal(11,0)  DEFAULT 0,
		added_eom_adjustment decimal(11,0)  DEFAULT 0,
		added_all_dues decimal(11,0) GENERATED ALWAYS AS (added_primary_dues + added_instalment + added_penalty_dues + added_eom_adjustment) STORED,
		closing_dues_account decimal(11,0)  DEFAULT 0,
		unadjusted_penalty_dues decimal(11,0)  DEFAULT 0, 
		...
		opening_eom_adjustment decimal(11,0) DEFAULT 0,
		opening_all_dues decimal(11,0) GENERATED ALWAYS AS (opening_primary_dues + opening_instalment + opening_penalty_dues + opening_eom_adjustment) STORED,
		knocked_off	decimal(11,0) GENERATED ALWAYS AS (opening_all_dues + added_all_dues + added_eom_adjustment - closing_dues_account) STORED,
		...
		closing_eom_adjustment decimal(11,0)  DEFAULT 0,
		closing_instalment decimal(11,0) DEFAULT 0,
		closing_all_dues decimal(11,0) GENERATED ALWAYS AS (closing_primary_dues + closing_instalment + closing_penalty_dues + closing_eom_adjustment) STORED,
	    ...
	    next_closing_instalment_principal decimal(11,0)  DEFAULT NULL,
	    next_closing_instalment_interest decimal(11,0)  DEFAULT NULL,
	    next_closing_instalment_instalment decimal(11,0)  DEFAULT NULL,
        next_closing_instalment decimal(11,0)  DEFAULT NULL,
   	    closing_principal decimal(11,0)  DEFAULT 0,	
	    next_closing_principal decimal(11,0)  DEFAULT NULL,	  		
		PRIMARY KEY id (id),
		KEY row_num (row_num),
	...
	CREATE TEMPORARY TABLE t_adjustments (
		...
		cum_amount decimal(11,0)  DEFAULT 0,
		PRIMARY KEY id (id),
		KEY obs_date (obs_date)
	) ENGINE=InnoDB AUTO_INCREMENT=32 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
update_table.closing_instalment=data.closing_instalment;
	...
	#select nach_id
	select @nach_id:= (select meta_value as nach_id from t_sublan_entries where object_id=@object_id and meta_key='nach_id');
	...
	select @agreement_id:= (select meta_value as agreement_id from t_sublan_entries where object_id=@object_id and  (meta_key='agreement_id' or meta_key='ops_process_id'));
	#agreement_process
	UPDATE t_loan_monthly_obs as update_table
	join
	(
...
DELIMITER $$
CREATE DEFINER=`root`@`10.102.50.174` PROCEDURE `seed_loan_dataset`()
BEGIN
set @id=0;
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_all_apps_dataset`()
BEGIN
SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED;
  ...
  */
  ...
  ON full_data.object_id=lapp_meta.object_id and meta_key='is_archived' where (meta_value is null or meta_value <> 'yes');
	...
	join 
	(
	select t_data.object_id, ifnull(meta_value,'') as lan_id from t_data join lapp_meta on  t_data.object_id=lapp_meta.object_id where coll_id='lapp' and meta_key='lan_id' 
	) as q1
	...
	q1 as (
	SELECT q0.lan_id,sublan.object_id as sublan_id from q0 join sublan ON q0.lan_id=sublan.meta_value  where coll_id='sublan' and meta_key='lan_id'
	),
	q2 as (
	SELECT q1.* from q1 join sublan ON q1.sublan_id=sublan.object_id where coll_id='sublan' and meta_key='ops_process' and meta_value='active'
	),
	q3 as (
	SELECT q2.*, meta_value as process_id from q2 JOIN sublan ON q2.sublan_id=sublan.object_id where coll_id='sublan' and meta_key='ops_process_id'
	)
	...
	#Delete all data which is no more there in primary table
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='channel_code'
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='rcode'
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='req_amount' AND meta_value <> ''
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='req_tenure' AND meta_value <> ''
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='req_scheme_id'
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='req_product_id'
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='loan_city'
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_source'
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_medium'
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_source_medium'
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_referrer'
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_campaign'
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_campaign_id'
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='lapp_stage'
	) as lapp_meta
	...
	join 
	( 
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data left join lapp_meta on lapp_meta.object_id=t_data.object_id and coll_id='lapp' AND meta_key='lapp_status' 
	) as lapp_meta  
	ON 
	update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='lapp_status_label' AND meta_value <> ''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='nbfc'
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(date_format(CONVERT(meta_value, DATE),'%Y%m%d'),'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='dob' and meta_value <> ''
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='educational_qualification'
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='employer_name'
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='employment_year'
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='fixed_income' AND meta_value <> ''
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='gender'
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='marital_status'
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='job_type'
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(left(meta_value,3),'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='cibil_score' AND meta_value <> ''
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='crif_score' AND meta_value <> ''
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='read_bank_statement'
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='auto_rejection_rule'
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='auto_approval'
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='delivery_squad' and meta_value <> ''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='lead_id' and meta_value <> ''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
  ...
  # utm_device
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_device'
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_mobile_os'
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_term'
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='gclid'
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='customer_id'
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='dob' and meta_value<>''
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='employment_duration' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='emi_outflow' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='rent_outflow' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_ad' and meta_value<>''
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_adset' and meta_value<>''
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_ad_id' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#utm_time 
	UPDATE data_store.all_apps_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='utm_time' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='lapp_datetime' and meta_value <> ''
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	SET update_table.lapp_datetime=lapp_meta.meta_value; 

	#dataset_timestamp
	UPDATE data_store.all_apps_dataset AS update_table
	join
	(
	select t_data.object_id, max(stamp) as meta_value from t_data join lapp_meta on t_data.object_id=lapp_meta.object_id group by lapp_meta.object_id
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'no') as meta_value from  t_data left join lapp_meta on t_data.object_id=lapp_meta.object_id and coll_id='lapp' AND meta_key='is_archived' 
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id,meta_value from  t_data left join lapp_meta on t_data.object_id=lapp_meta.object_id and coll_id='lapp' AND meta_key='relationship_manager'  and meta_value <>''
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	  select stamp as kyc_completed_dt,t_data.object_id as lapp_id from t_data join lapp_meta on t_data.object_id=lapp_meta.object_id where coll_id='lapp' and meta_key='kyc_verified' and meta_value = 'yes'
	)as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.lapp_id
	...
	join
	(
	select t_data.object_id, meta_value from t_data join lapp_meta  on lapp_meta.object_id=t_data.object_id where coll_id='lapp' AND meta_key='utm_origin' and meta_value  <> ''
	) as lapp_meta
	...
	join
	(
	select t_data.object_id, meta_value from t_data join lapp_meta on lapp_meta.object_id=t_data.object_id where coll_id='lapp' 		AND meta_key='utm_origin_flow' and meta_value  <> ''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, meta_value from t_data join lapp_meta on  lapp_meta.object_id=t_data.object_id  where coll_id='lapp' and meta_key='job_subtype' and meta_value<>''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, meta_value from t_data join lapp_meta  on  lapp_meta.object_id=t_data.object_id where coll_id='lapp' AND meta_key='end_use' and meta_value<>''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id,stamp, meta_value from t_data join lapp_meta  on  lapp_meta.object_id=t_data.object_id where coll_id='lapp' AND meta_key='doc_combo' and meta_value<>''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id,  meta_value from t_data join lapp_meta  on  lapp_meta.object_id=t_data.object_id where coll_id='lapp' AND meta_key='kyc_method' and meta_value<>''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, meta_value from t_data join lapp_meta  on  lapp_meta.object_id=t_data.object_id where coll_id='lapp' AND meta_key='no_offers_reason' and meta_value<>''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, meta_value from t_data join lapp_meta  on  lapp_meta.object_id=t_data.object_id where coll_id='lapp' AND meta_key='no_offers_id' and meta_value<>''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, meta_value from t_data join lapp_meta  on  lapp_meta.object_id=t_data.object_id where coll_id='lapp' AND meta_key='equifax_score' and meta_value<>''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, meta_value from t_data join lapp_meta  on  lapp_meta.object_id=t_data.object_id where coll_id='lapp' AND meta_key='equifax_writeoff' and meta_value<>''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, meta_value from t_data join lapp_meta  on  lapp_meta.object_id=t_data.object_id where coll_id='lapp' AND meta_key='equifax_overdue' and meta_value<>''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, meta_value from t_data join lapp_meta  on  lapp_meta.object_id=t_data.object_id where coll_id='lapp' AND meta_key='equifax_temp_rejection' and meta_value<>''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, meta_value from t_data join lapp_meta  on  lapp_meta.object_id=t_data.object_id where coll_id='lapp' AND meta_key='equifax_temp_reason' and meta_value<>''
	) as lapp_meta
	ON
	update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta on t_data.object_id=lapp_meta.object_id where coll_id='lapp' AND meta_key='mapp_id'
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
    ...
    join
    (
        select t_data.object_id, meta_value from t_data join lapp_meta on t_data.object_id=lapp_meta.object_id where coll_id='lapp' AND meta_key='credit_score' AND meta_value <> ''
    ) as lapp_meta
    ON
    update_table.lapp_id=lapp_meta.object_id
    ...
    join
    (
        select t_data.object_id, meta_value from t_data join lapp_meta on t_data.object_id=lapp_meta.object_id where coll_id='lapp' AND meta_key='non_financial_credit_score' AND meta_value <> ''
    ) as lapp_meta
    ON
    update_table.lapp_id=lapp_meta.object_id
    ...
    join
    (
        select t_data.object_id, meta_value from t_data join lapp_meta on t_data.object_id=lapp_meta.object_id where coll_id='lapp' AND meta_key='preferred_agreement_method'
    ) as lapp_meta
    ON
    update_table.lapp_id=lapp_meta.object_id
    ...
    join
    (
        select t_data.object_id, meta_value from t_data join lapp_meta on t_data.object_id=lapp_meta.object_id where coll_id='lapp' AND meta_key='lt_generic_fail '
    ) as lapp_meta
    ON
    update_table.lapp_id=lapp_meta.object_id
	...
	join(  
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='full_name' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='personal_email' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='mobile_number' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#pan_card 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='pan_card' and meta_value<>'' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='home_addr_line1' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='home_addr_line2' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='home_city' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='home_zipcode' and meta_value<>''
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#office_addr_line1 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='office_addr_line1' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='office_addr_line2' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='office_city' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#office_zipcode 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='office_zipcode' and meta_value<>''
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='office_landline_no' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='official_email' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='company_mca_id' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='ip' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='salary_account_no' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='salary_bank_name' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='ecs_cust_name' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='ecs_bank' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#ecs_bank_acc_no 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='ecs_bank_acc_no' and meta_value<>''
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#ecs_ifsc_code 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='ecs_ifsc_code' and meta_value<>''
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#business_name 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='business_name' and meta_value<>''
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#business_address 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='business_address' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#business_addr_line1 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='business_addr_line1' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#business_addr_line2 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='business_addr_line2' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#business_city 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='business_city' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#business_zipcode 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='business_zipcode' and meta_value<>''
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='business_monthly_sales' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#business_monthly_gp 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='business_monthly_gp' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	#business_monthly_np 
	UPDATE data_store.app_personal_dataset AS update_table 
	join( select t_data.object_id, ifnull(meta_value,'') as meta_value from t_data join lapp_meta using(object_id) where coll_id='lapp' AND meta_key='business_monthly_np' 
	) as lapp_meta 
	ON update_table.lapp_id=lapp_meta.object_id 
	...
	join
	(
	select t_data.object_id,meta_value from  t_data left join lapp_meta on t_data.object_id=lapp_meta.object_id and coll_id='lapp' AND meta_key='fathers_name'  and meta_value <>''
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id,meta_value from  t_data left join lapp_meta on t_data.object_id=lapp_meta.object_id and coll_id='lapp' AND meta_key='mothers_maiden_name'  and meta_value <>''
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id,meta_value from  t_data left join lapp_meta on t_data.object_id=lapp_meta.object_id and coll_id='lapp' AND meta_key='aadhar_uid '  and meta_value <>''
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id,meta_value from  t_data left join lapp_meta on t_data.object_id=lapp_meta.object_id and coll_id='lapp' AND meta_key='permanent_addr_line1'  and meta_value <>''
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id,meta_value from  t_data left join lapp_meta on t_data.object_id=lapp_meta.object_id and coll_id='lapp' AND meta_key='permanent_addr_line2'  and meta_value <>''
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id,meta_value from  t_data left join lapp_meta on t_data.object_id=lapp_meta.object_id and coll_id='lapp' AND meta_key='permanent_city'  and meta_value <>''
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
	...
	join
	(
	select t_data.object_id,meta_value from  t_data left join lapp_meta on t_data.object_id=lapp_meta.object_id and coll_id='lapp' AND meta_key='permanent_zipcode'  and meta_value <>''
	) as lapp_meta
	ON update_table.lapp_id=lapp_meta.object_id
